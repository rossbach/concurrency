...
#ifndef __KMEANS_RAW_CU_H__
#define __KMEANS_RAW_CU_H__
...
__global__ void finalizeCentersColumnMajor(float * pC, int * pCC) {
	int idx = blockIdx.x*blockDim.x+threadIdx.x;
	if(idx >= C) return;
    // float pC0 = pC[idx+(0*C)];
    // float pC1 = pC[idx+(1*C)];
    // float pC2 = pC[idx+(2*C)];
    int nNumerator = pCC[idx];
    if(nNumerator) {
	    for(int i=0;i<R;i++) {
		    pC[idx+(i*C)] /= pCC[idx];
    ...
    // printf("finalize: pC[%d]=[%.3f, %.3f, %.3f...]/%d=>[%.3f, %.3f, %.3f...]\n",
    //         idx,
    //         pC0,
    //         pC1,
    //         pC2,
...
void accumulateSM_RCeqBlockSize(float * pP, float * pC, int * pCC, int * pCI, int nP) {
    ...
    __syncthreads();
    atomicAdd(&pC[threadIdx.x], accums[threadIdx.x]);
...
void accumulateSM(float * pP, float * pC, int * pCC, int * pCI, int nP) {
    __shared__ float accums[R*C];
    __shared__ int cnts[C];
    dassert(R*C*sizeof(float) <= SHMEMSIZE);
    if(threadIdx.x < C) {
        cnts[threadIdx.x] = 0;
        for(int i=0;i<R;i++) 
...
protected:
    ...
    static void accumulateRCeqBS(float * pP, float * pC, int * pCC, int * pCI, int nP) {
        // if R*C < max thread group, we can reset and merge
        // accumulators in a more efficient way than if R*C is larger
        const int nBlockSize = min(R*C, MAXBLOCKTHREADS);
    ...
    static void accumulateGeneral(float * pP, float * pC, int * pCC, int * pCI, int nP) {
        const int nMinAccumBlocks = iDivUp(C, THREADBLOCK_SIZE);
        const int nMinPointsBlocks = iDivUp(nP, THREADBLOCK_SIZE);
        const int nBlocks = max(nMinAccumBlocks, nMinPointsBlocks);
        if(ROWMAJ) {
            accumulateSM<R,C><<<nBlocks, THREADBLOCK_SIZE>>>(pP, pC, pCC, pCI, nP); 
        } else {
