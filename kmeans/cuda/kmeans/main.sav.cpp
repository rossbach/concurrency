...
declare_testsuite_hdrs(24, 64);
std::map<std::string, std::map<int, std::map<int, LPFNKMEANS>>> g_lpfns;
bool g_blpfnInit = false;
decl_init_lpfn_table_begin(g_lpfns,g_blpfnInit);
    create_testsuite_entries(g_lpfns, 24, 64);
decl_init_lpfn_table_end(g_lpfns, g_blpfnInit);
declare_lpfn_finder(g_lpfns, g_blpfnInit)             

int gVerbose = 0;
// #define INFORM(b,x) { if(b) {  shrLog(x); } }

double kmeans_simple(const int nSteps,
                     void * h_Points,
                     void * h_Centers,
	                 const int nPoints,
	                 const int nCenters,
...
double    kmeans_raw(const int nSteps,
                     void * h_Points,
                     void * h_Centers,
	                 const int nPoints,
	                 const int nCenters,
...
double    kmeans_cm_r24_c64(const int nSteps,
                    void * h_Points,
                    void * h_Centers,
	                const int nPoints,
	                const int nCenters,
...
double    kmeans_cm_constmem(const int nSteps,
                             void * h_Points,
                             void * h_Centers,
	                         const int nPoints,
	                         const int nCenters,
...
double    kmeans_cm_constmem_shr(const int nSteps,
                                 void * h_Points,
                                 void * h_Centers,
	                             const int nPoints,
	                             const int nCenters,
...
double    kmeans_cm_constmem_shr_map(const int nSteps,
                                     void * h_Points,
                                     void * h_Centers,
	                                 const int nPoints,
	                                 const int nCenters,
...
double kmeans_constmem(const int nSteps,
                       void * h_Points,
                       void * h_Centers,
	                   const int nPoints,
	                   const int nCenters,
...
double kmeans_constmem_memsetreset(const int nSteps,
                                   void * h_Points,
                                   void * h_Centers,
	                               const int nPoints,
	                               const int nCenters,
...
double kmeans_constmem_memsetreset_shr(const int nSteps,
                                       void * h_Points,
                                       void * h_Centers,
	                                   const int nPoints,
	                                   const int nCenters,
...
double kmeans_constmem_memsetreset_shr_map(const int nSteps,
                                           void * h_Points,
                                           void * h_Centers,
	                                       const int nPoints,
	                                       const int nCenters,
...
LPFNKMEANS g_lpfnKMeans = kmeans_simple;
...
int g_nDEFAULTRANK = DEFAULTRANK;
    ...
    )
{
    ...
    return find_lpfn(strName, 24, 64);
}
#else
///-------------------------------------------------------------------------------------------------
    ...
    )  
{
    ...
    while((c = getopt(argc, argv, "vVs:t:f:hr:")) != -1) {
        switch(c) {	
        case 'h': return 0;
        ...
        case 't':
            ...
            ::g_lpfnKMeans = choose_kmeans_impl(g_vKMeansVersion);
            break;
        case 'f':
            if(optarg == NULL) {
...
int main(int argc, char **argv)
{
    ...
    int numCoords = 0;
    int nDefClusterId = 0;
    std::vector<pt<DEFAULTRANK>> points;
    std::vector<pt<DEFAULTRANK>> centers;
    std::vector<pt<DEFAULTRANK>> refcenters;
    ...
    ReadInput<DEFAULTRANK>(g_vInputFile, points, &numObjs, &numCoords, 0);
	if (points.size()==0) {
    ...
    ChooseInitialCenters(points,                   // points to choose from              
                         ...
                         DEFAULTCENTERS,               // number of points to choose from.
                         g_nSeed);                 // random seed. ACHTUNG! Beware benchmarkers

	int nPoints = (int)points.size();
	int nCenters = DEFAULTCENTERS;
	size_t uiPointsBytes = nPoints * DEFAULTRANK * sizeof(float);
	size_t uiCentersBytes = DEFAULTCENTERS * DEFAULTRANK * sizeof(float);
	size_t uiClusterIdsBytes = nPoints * sizeof(int);
	size_t uiClusterCountsBytes = nCenters * sizeof(int);

	pt<DEFAULTRANK> *h_Points = (pt<DEFAULTRANK>*)malloc(uiPointsBytes);
	pt<DEFAULTRANK> *h_Centers = (pt<DEFAULTRANK>*)malloc(uiCentersBytes);
	int * h_ClusterIds = (int*)malloc(uiClusterIdsBytes);
	memset(h_ClusterIds, 0, uiClusterIdsBytes);
	std::vector<pt<DEFAULTRANK>>::iterator vi;
	pt<DEFAULTRANK>* pPoints = h_Points;
	for(vi=points.begin(); vi!=points.end(); vi++) 
		*pPoints++ = *vi;
	pt<DEFAULTRANK>* pCenters = h_Centers;
	for(vi=centers.begin(); vi!=centers.end(); vi++) 
		*pCenters++ = *vi;
	...
	if(g_bVerify) {
		...
		for(vi=centers.begin(); vi!=centers.end(); vi++) 
			*vi = *pCenters++;
		bool bSuccess = CompareResults<DEFAULTRANK>(centers, refcenters,  0.1f, g_bVerbose);
		PrintResults<DEFAULTRANK>(stdout, centers, refcenters, bSuccess, dAvgSecs, dRefAvgSecs, g_bVerbose);
	}
