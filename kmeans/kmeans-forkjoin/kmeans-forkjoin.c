...
int     _printres=1;       /* print result option */
...
void kmeans(int id, int nprocs) {
    ...
    */
	...
	_counts[i] += counts[i];
      ...
      */
      ...
      barrier_wait_ex(_barrierX, _barrierY, id, nprocs);
	...
	*_converged = ((!centers_changed(_ncenters)) || itr >= max_itr);
	reset_accumulators(_sums, _counts);
      ...
      barrier_wait_ex(_barrierY, _barrierX, id, nprocs);
...
void main (int argc, char *const *argv) {
    ...
    while ((opt = getopt(argc, argv, "k:t:i:vc:s")) != -1) {
	switch (opt) {
	case 'k':
	    _k = atoi(optarg);
	    break;
